...
Graph *create_graph() {
    ...
    graph->faces = malloc((2+ graph->capacity_edges + graph->capacity_vertices) * sizeof(Path));
    if (graph->faces == NULL) {
        free(graph->neighbors);
        free(graph->vertices);
        free(graph);
        perror("Failed to allocate memory for Graph.faces in create_graph");
        exit(1);
    }
...
void create_edge(Graph *g, const int v1_id, const int v2_id, const int weight) {
    ...
    if (g->nb_edges == g->capacity_edges) {
        ...
        Path *tmp2 = realloc(g->faces, (2 + g->capacity_edges + g->capacity_vertices) * sizeof(Path));
        if (tmp2 == NULL) {
            perror("Failed to reallocate memory for Graph.faces in create_edge");
            exit(1);
        }
        g->faces = tmp2;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        const double angle = atan2l(g->vertices[ids[(i + 1) % 2]].y - g->vertices[ids[i]].y,
        ...
        if (nl->count == nl->capacity) {
            ...
            nl->neighbors = tmp;
            double *tmp2 = realloc(nl->angles, nl->capacity * sizeof(double));
            if (tmp2 == NULL) {
                perror("Failed to reallocate memory for Graph.neighbors angles in create_edge");
                exit(1);
            }
            nl->angles = tmp2;
        }
        ...
        // ✓ Écrire les deux valeurs à la position finale
    ...
    g->edges[g->nb_edges] = (Edge){v1_id, v2_id, weight, g->nb_edges, g->nb_edges, 0, 0, {-1, -1}};
    g->nb_edges++;
}
...
void delete_edge(Graph *g, const int e_id) {
    ...
    // Retirer des listes de voisins
    const int u = g->edges[e_id].u;
    const int v = g->edges[e_id].v;
    // retirer v des voisins de u
    for (int i = 0; i < g->neighbors[u].count; i++) {
        if (g->neighbors[u].neighbors[i] == v) {
            g->neighbors[u].neighbors[i] = g->neighbors[u].neighbors[--g->neighbors[u].count];
            g->neighbors[u].angles[i] = g->neighbors[u].angles[--g->neighbors[u].count];
            break;
        }
    }
    // retirer u des voisins de v
    for (int i = 0; i < g->neighbors[v].count; i++) {
        if (g->neighbors[v].neighbors[i] == u) {
            g->neighbors[v].neighbors[i] = g->neighbors[v].neighbors[--g->neighbors[v].count];
            g->neighbors[v].angles[i] = g->neighbors[v].angles[--g->neighbors[v].count];
            break;
        }
    }
}
...
void create_vertex(Graph *g, const double x, const double y) {
    ...
    if (g->nb_vertex == g->capacity_vertices) {
        ...
        Path *tmp3 = realloc(g->faces, (2 + g->capacity_edges + g->capacity_vertices) * sizeof(Path));
        if (tmp3 == NULL) {
            perror("Failed to reallocate memory for Graph.faces in create_vertex");
            exit(1);
        }
        g->faces = tmp3;
    }
...
void sort_neighbors_by_atan(const Graph *g, const int v_id) {
    Neighbor_list *nl = &g->neighbors[v_id];
    for (int i = 0; i < nl->count - 1; i++) {
        for (int j = 0; j < nl->count - i - 1; j++) {
            if (nl->angles[j] > nl->angles[j + 1]) {
                // Swap neighbors
                const int temp_neighbor = nl->neighbors[j];
                nl->neighbors[j] = nl->neighbors[j + 1];
                nl->neighbors[j + 1] = temp_neighbor;

                // Swap angles
                const double temp_angle = nl->angles[j];
                nl->angles[j] = nl->angles[j + 1];
                nl->angles[j + 1] = temp_angle;
            }
        }
    }
}

void move_vertex(Graph *g, const int v, const int new_x, const int new_y) {
...
void delete_vertex(Graph *g, const int v_id) {
    ...
    // Supprimer aussi toutes les arêtes incidentes
    for (int e = 0; e < g->nb_edges; e++) {
...
void split_edge(Graph *g, const int edge_id, const int number_vertex_to_add) {
    ...
    const double u_y = g->vertices[u].y;
    const double v_x = g->vertices[v].x;
    const double v_y = g->vertices[v].y;
...
void split_edges(Graph *g, const int *edge_ids, const int number_edge_to_split, const int number_vertex_to_add) {

    int* nb_vertex_to_add_per_edge = malloc(number_edge_to_split * sizeof(int));
    if (!nb_vertex_to_add_per_edge) {
        perror("Failed to allocate memory for nb_vertex_to_add_per_edge in split_edge");
        exit(1);
    }

    for (int i=0; i<number_vertex_to_add;i++) {
        const int random_edge = rand() % number_edge_to_split;
        nb_vertex_to_add_per_edge[i] = g->edges[random_edge].u;
    }

    for (int i=0; i<number_edge_to_split;i++) {
        split_edge(g, edge_ids[i], nb_vertex_to_add_per_edge[i]);
    }

    free(nb_vertex_to_add_per_edge);
}
...
void compute_all_shortest_paths(const Graph *g) {
    ...
    for (int s = 0; s < g->nb_vertex; s++) {
        ...
        while (front < rear) {
            ...
            // and if so, we update the distance
            for (int i = 0; i < g->neighbors[u].count; i++) {
            ...
            if (dist != -1 && u != v) {
                ...
                while (dist > 0) {
                    ...
                    for (int i = 0; i < g->neighbors[current].count; i++) {
                        const int neighbor = g->neighbors[current].neighbors[i];
                        if (g->vertices[neighbor].deleted) continue;
                        if (g->distances[u * g->nb_vertex + neighbor] == dist - 1) {
                            if (best == -1) best = neighbor;
                            else {
                                if (g->vertices[best].label > g->vertices[neighbor].label) {
                                    best = neighbor;
                                }
                            }
                        }
                    ...
                    if (best == -1) {
                        fprintf(stderr, "Error in compute_all_shortest_paths: no predecessor found for vertex %d in the path from %d to %d, should not happen if the function is called correctly\n", current, u, v);
                        exit(1);
                    }
...
void find_outer_face(Graph *g) {
    ...
    for (int i = 1; i < g->nb_vertex; i++) {
        if (g->vertices[i].deleted) continue;
        if (g->vertices[i].x < g->vertices[leftmost_vertex].x ) leftmost_vertex = i;
    }

    int u = leftmost_vertex;
    int v = g->neighbors[leftmost_vertex].neighbors[0];

    do {
        const int eid = g->edge_indices[u * g->nb_vertex + v];
        if (eid >= 0)
            g->edges[eid].is_outer = 1;
        ...
        u = v;
        v = w;
    } while (u != leftmost_vertex || v != g->neighbors[leftmost_vertex].neighbors[0]);

    for (int f = 0; f < g->nb_faces; f++) {
        int has_outer_edge = 1;
        for (int e = 0; e < g->nb_edges; e++) {
            if (g->faces[f].edges_ids[e] == 1 && !g->edges[e].is_outer) {
                has_outer_edge = 0;
                break;
            }
        }
...
void find_faces(Graph *g) {
    ...
    if (!visited) {
        perror("Failed to allocate visited in find_faces");
        exit(1);
    }
    ...
    for (int v = 0; v < n; v++) {
        ...
        for (int i = 0; i < g->neighbors[v].count; i++) {
            const int neighbor = g->neighbors[v].neighbors[i];

            if (visited[v * n + neighbor]) continue;
            ...
            face->length = 0;
            if (!face->edges_ids || !face->vertices_ids || !face->edges_labels) {
                perror("Failed to allocate face path in find_faces");
                exit(1);
            }
            ...
            while (!visited[u * n + p]) {
                ...
                if (eid >= 0) {
                    ...
                    } else if (g->edges[eid].face_id[1] == -1) {
                        g->edges[eid].face_id[1] = g->nb_faces;
                    } else {
                        fprintf(stderr, "Error in find_faces: edge %d already has two faces,"
                                        " should not happen if the graph is planar\n", eid);
                        exit(1);
                    }
                }
                ...
                if (idx < 0) {
                    fprintf(stderr, "find_faces: sommet %d absent de la liste de %d\n", u, p);
                    exit(1);
                }
    ...
    for (int v = 0; v < g->nb_vertex; v++) {
        ...
        for (int e = 0; e < g->nb_edges; e++) {
            ...
            if (memcmp(expected, actual, g->nb_vertex * sizeof(uint32_t)) == 0) {
                ...
                uint32_t *edges_labels = calloc(g->nb_edges, sizeof(uint32_t));
                if (!edges_of_c) {
                    free(expected);
                    free(actual);
                ...
                if (!edges_labels) {
                    ...
                    free(edges_of_c); free(vertices_of_c);
                    perror("Failed to allocate memory for edges_labels in find_horton_cycles");
                    exit(1);
                }
                for (int i = 0; i < g->nb_edges; i++) {
                    edges_of_c[i] = path_v_to_x.edges_ids[i] ^ path_v_to_y.edges_ids[i];
                    if (edges_of_c[i]) {
                        edges_labels[g->edges[i].label] = 1;
                    }
                }
                for (int i = 0; i < g->nb_vertex; i++) {
                ...
                c.edges_ids[e] = 1;
                edges_labels[g->edges[e].label] = 1;
                c.edges_labels = edges_labels;
                c.vertices_ids = vertices_of_c;
                c.length = path_v_to_x.length + path_v_to_y.length + 1;
                add_horton_cycle(g, c);
            }
            free(path_v_to_x.edges_ids); free(path_v_to_x.vertices_ids);
            free(path_v_to_y.edges_ids); free(path_v_to_y.vertices_ids);
            free(expected);
            free(actual);
        }
    }
...
int compare_cycles_permutations(const void *a, const void *b) {
    ...
    for (int i = 0; i < nb_edges_for_qsort; i++) {
        if (c1->edges_labels[i] != c2->edges_labels[i])
            return c1->edges_labels[i] - c2->edges_labels[i];
    }
    return 0;
}
...
void greedy_cycle_cover(Graph *g) {
    ...
    for (int i = 0; i < g->nb_horton_cycles; i++) {
        ...
        if (independent) {
            ...
            memcpy(mcb[mcb_count].edges_labels, candidate.edges_labels, g->nb_edges * sizeof(uint32_t));
