...
Dans un graphe $G=(V,E)$, une chaîne $P$ est une suite d'arêtes $(e_0, e_1,\ldots,e_n-1)$ telle que chaque paire d'arêtes successives  $(e_i, e_{i+1}) \forall i \in 0..n-2$ partage un sommet en commun. La longueur de la chaîne est $n$.
\end{definition}
...
Cependant, au vu du grand nombre de graphes obtenus, il est impossible de tous les étudier. Un choix a donc été fait: celui de garder uniquement les graphes pour lesquels, dans chaque base de cycles minimale trouvée, au moins une arête appartenait à au moins 3 cycles de la base, et ce pour chaque base. En effet, une arête sépare exactement deux faces dans un graphe planaire. Donc, s'il existe dans chaque base une arête qui se trouve dans au moins 3 cycles, cela signifie qu'au moins un de ces trois cycles n'est pas une face et que, donc, aucune base de cycles minimales ne correspond à l'ensemble des faces. \\
...
Soit un graphe $G=(V,E)$ ayant $x$ chaînes (avec $x \ge 4$) notées $P_1, P_2,\ldots, P_x$ allant d'un sommet $A$ à un sommet $B$. L'intersection de chacune de ces chaînes deux à deux donne l'ensemble $\{A,B\}$, c'est à dire que les seuls sommets communs à ces chaînes sont leurs extrémités soient les sommets $A$ et $B$. \\

La chaîne $P_1$ a une longueur $n$ avec $n \ge 1$ et les autres chaînes $P_i$ ont une longueur $k_i$ avec $k_i > n$. Le graphe présenté à la figure \ref{homéomorphe-multigraphe} correspond à ces critères. Pour la démonstration, on supposera que $k_i= n+1$ pour tout $i$ allant de 2 à $x$. \\
Une arête peut au maximum appartenir à deux faces or chacune des chaînes reliant $A$ et $B$ peut être considérée comme une arête après lissage. Donc chacune de ses chaînes peut au maximum appartenir à deux faces \\

Si on souhaite former la base de cycles $\mathcal{B}_{{\text{faces}}}$ formée par toutes les faces du graphe tout en minimisant $\ell(\mathcal{B}_{{\text{faces}}})$, il faut donc que la chaîne la plus courte appartienne à deux faces intérieures du graphe. Ces deux faces seront constituées de $P_1$ et d'une autre chaîne $P_i$ et auront donc une longueur de $n+n+1=2n+1$. \\

$x$ chaînes forment $x-1$ faces. Il reste donc $x-1-2=x-3$ faces à former. Ces dernières ne peuvent être composées que des deux chaînes $P_i$ et $P_j$ avec $i > j > 1$. Ces faces auront donc une longueur de $2(n+1) = 2n+2$. Si on forme une base de cycles avec ces faces,  $\ell(\mathcal{B}_{{\text{faces}}}) = 2(2n+1)+(x-3)(2n+2) = (4n+2)+(2nx+2x-6n-6) = 2nx+2x-2n-4 $. \\

On tente de former une base que l'on nomme $\mathcal{B}_{{\text{optimal}}}$ avec $\ell(\mathcal{B}_{{\text{optimal}}}) < \ell(\mathcal{B}_{{\text{faces}}})$. On a $x$ chaînes qui forment $x-1$ faces intérieurs. Donc pour former une base, on a besoin de $x-1$ cycles indépendants (voir \ref{nb_cyclo}). On forme les cycles en combinant $P_1$ à $P_i$ pour $i$ allant de 2 à $x$. On a donc $x-1$ cycles de longueur $n + n + 1 = 2n+1$. On a donc $\ell(\mathcal{B}_{{\text{optimal}}}) = (x-1)(2n+1) = 2nx + x - 2n - 1$. \\

Si on soustrait $\ell(\mathcal{B}_{{\text{optimal}}})$ à $\ell(\mathcal{B}_{{\text{faces}}})$ on obtient: $\ell(\mathcal{B}_{{\text{faces}}}) - \ell(\mathcal{B}_{{\text{optimal}}}) = (2nx+2x-2n-4) - ( 2nx + x - 2n - 1)=x-3$. Donc dès que $x > 3$, la différence entre $\ell(\mathcal{B}_{{\text{optimal}}})$ et $\ell(\mathcal{B}_{{\text{faces}}})$ devient positive. Donc $\ell(\mathcal{B}_{{\text{optimal}}}) < \ell(\mathcal{B}_{{\text{faces}}})$ pour $x > 3$. Donc quel que soit $n$, la base de cycles $\mathcal{B}_{{\text{optimal}}}$ contient moins d'arêtes que la base $\mathcal{B}_{{\text{faces}}}$. \\

Soit $C$ l'ensemble de cycles tel que pour tout $C_i$ dans $C$, avec $i$ un entier compris entre 2 et $x$ inclus, $C_i = P_i \cup  P_1 $. Montrons que $C$ est une base de cycles. Le nombre cyclomatique est égal au nombre de faces intérieures. Or, pour les graphes décrits précédemment, le nombre de faces est égal à $x-1$ avec $x$ le nombre de chaînes. Or, en combinant les $x-1$ chaînes de longueur supérieure à $n-1$ avec celle de longueur $n$, on forme bel et bien $x-1$ cycles. \\

De plus, on voit que chaque chemin $P_i \forall i \in 2..x$ est composé d'arêtes qui lui sont propres et qui ne sont présentes dans aucune autre chaîne. Le cycle $C_i$ composé de $P_i$ et $P_1$ est donc le seul cycle de la base à contenir les arêtes de la chaîne $P_i$. Il est donc impossible de trouver un cycle ayant des arêtes de $P_i$ par une combinaison des autres cycles. Donc les cycles sont bien indépendants entre eux. Or on a prouvé précédemment que $C$ comportait $x-1$ cycles donc d'après \ref{nb_cyclo}, $C$ forme bien une base de cycle. \\

En conclusion, $\mathcal{B}_{{\text{faces}}}$ ne forme pas une base de cycle minimale pour aucun graphe ayant un graphe de l'ensemble décrit comme sous-graphe.
...
Malgré cela, cette limite n'est valable que pour les graphes qui ne sont pas connexe puis que le théorème de Whitney affirme que tous les graphes planaires 3-connexes admettent un unique plongement sur un plan.
...
Il existe une famille de graphes pour lesquels les faces ne forment pas la base de cycles minimale. Cependant, il se peut que d'autres familles existent, ou bien, que certaines contraintes données dans la caractérisation puissent être relâchées. \\
Pour des travaux futurs, il serait intéressant de regarder le nombre de bases de cycles minimales lorsque les faces n'en sont pas une.
...
\begin{appendices}
\label{annexe}
\section{Exemples de graphes dont les faces internes de forment pas une base de cycle minimale}
