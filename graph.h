...
typedef struct {
    ...
    neighbor vertex in graph->vertices */
    int count; /**< Number of neighbors currently stored in the list */
    int capacity; /**< Capacity of the allocated neighbors array */
} Neighbor_list;

/**
 * @brief Cycle in the graph.
 *
 * Represents a cycle in the graph. The edges_ids array is a binary vector of size nb_edges, where
 * edges_ids[i] = 1 if the edge with index i in graph->edges is part of the cycle, and 0 otherwise.
 * The length is the number of vertices in the cycle, and label_weight is the sum of the labels of
 * the edges in the cycle, used for ordering cycles in Horton's algorithm when two cycles have the
 * same length.
 */
typedef struct {
    uint32_t *edges_ids;
    /**< Binary vector of size nb_edges, where edges_ids[i] = 1 if the edge with index i in
     *graph->edges is part of the cycle, and 0 otherwise */
    int length; /**< Number of vertices in the cycle */
    int weight; /**< Sum of the labels of the edges in the cycle, used for ordering
    cycles in Horton's algorithm when two cycles have the same length */
} Cycle;
 ...
 */
typedef struct {
    Cycle *cycles; /**< Array of cycles in the minimal cycle basis, where cycles[i] is the (i+1)-th
    cycle of the basis */
} Minimal_basis;
...
typedef struct {
    ...
    Cycle *horton_cycles; /**< Dynamic array of cycles generated by Horton's algorithm,
